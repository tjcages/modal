...
import React__default, { useState, useEffect, useLayoutEffect, useMemo } from 'react';
        ...
        if (isOpen && shouldScaleBackground) {
            ...
            const wrapperStylesCleanup = assignStyle(wrapper, {
                ...
                ...isVertical(direction) ? {
                    transform: `scale(${getScale()}) translate3d(0, ${isDesktop ? '0' : 'calc(env(safe-area-inset-top) + 14px)'}, 0)`
                } : {
    ...
    function onDrag(event) {
        ...
        if (isDragging) {
            ...
            if (isDraggingInDirection && !snapPoints) {
                ...
                set(drawerRef.current, {
                    transform: isVertical(direction) ? `translate3d(${isDesktop ? '-50%' : '0'}, ${isDesktop ? `calc(-75% + ${translateValue}px)` : `${translateValue}px`}, 0)` : `translate3d(${translateValue}px, 0, 0)`
                });
                return;
            }
            ...
            if (!snapPoints) {
                ...
                set(drawerRef.current, {
                    transform: isVertical(direction) ? `translate3d(${isDesktop ? '-50%' : '0'}, ${isDesktop ? `calc(-75% + ${translateValue}px)` : `${translateValue}px`}, 0)` : `translate3d(${isDesktop ? `calc(-50% - ${translateValue}px)` : `${translateValue}px`}, ${isDesktop ? '-75%' : '0'}, 0)`
                });
            }
        }
    }
        ...
        if (shouldScaleBackground && currentSwipeAmount && currentSwipeAmount > 0 && isOpen) {
            set(wrapper, {
                ...
                ...isVertical(direction) ? {
                    ...
                    transform: `scale(${getScale()}) translate3d(${isDesktop ? '0' : 'calc(env(safe-area-inset-top) + 14px)'}, 0, 0)`,
...
const Content = /*#__PURE__*/ React__default.forwardRef(function({ onPointerDownOutside, style, onClick, ...rest }, ref) {
